...
class OctopusEnergyClient {
  ...
  /**
   * Determine if a time is in off-peak period for Intelligent Octopus Go
   * Off-peak: 11:30 PM - 5:30 AM (23:30 - 05:30)
   * @param {Date} date - Date object to check
   * @returns {boolean} True if in off-peak period
   */
  isOffPeak(date) {
    ...
    // 11:30 PM = 23:30 = 23*60 + 30 = 1410 minutes
    // 5:30 AM = 05:30 = 5*60 + 30 = 330 minutes
    const offPeakStart = 23 * 60 + 30; // 1410
    const offPeakEnd = 5 * 60 + 30;    // 330
    
    // Off-peak spans midnight, so check if time is after 23:30 OR before 05:30
    return timeInMinutes >= offPeakStart || timeInMinutes < offPeakEnd;
  }
  ...
  async importSessions(dateFrom, dateTo, threshold = 2.0, tariffRate = null) {
    ...
    // If no tariff rate provided, use intelligent detection based on time
    // Most EV charging happens during off-peak hours
    ...
    const transformedSessions = detectedSessions.map(session => {
      ...
      if (useAutoRate) {
        ...
        sessionRate = this.isOffPeak(startDate) ? 7.5 : 24.5;
        console.log(`Auto-detected rate for session starting ${session.startTime}: ${sessionRate}p/kWh`);
      }
