...
class OctopusEnergyClient {
  ...
  async importSessionsFromCompletedDispatches(options = {}) {
    ...
    return merged.map((session) => {
      ...
      const resolvedRate = typeof tariffResolver === 'function'
        ? Number(tariffResolver(startDate))
        : Number(tariffRate);
      const sessionRate = Number.isFinite(resolvedRate) ? resolvedRate : Number(tariffRate) || 7.0;
      const cost = (session.totalKwh * sessionRate) / 100;
      ...
      return {
        ...
        dispatchBlocks: session.dispatches.map((dispatch) => ({
          ...
          cost: parseFloat(((dispatch.chargedKwh * sessionRate) / 100).toFixed(2)),
